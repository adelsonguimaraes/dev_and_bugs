...
export class Controller {
    constructor() {
        ...
        this.bosses = [];
        this.dungeons = [];
        this.createBosses = () => __awaiter(this, void 0, void 0, function* () {
            const data = yield LoadData.get('./data/bosses.json');
            data.forEach((e) => this.bosses.push(Boss.fromJson(e)));
        });
        this.createDungeons = () => {
            this.dungeons = [
        ...
        this.controllerSequenceDrop = () => {
            if (this.level >= 30) {
                if (Alert.isAlerted(Alert.DROP_BUGS_3X))
                    return false;
                this.sequenceDrops = 3;
                Alert.displayEventInfo(Alert.DROP_BUGS_3X);
            }
        };
        this.incrementBugLifeUpdate = () => this.incrementBugLife += 3;
        this.inShooting = () => this.on;
        this.startShooting = () => this.on = true;
        this.stopShooting = () => this.on = false;
        this.displayLevel = () => {
        ...
        this.DOMContentLoaded = () => {
            document.addEventListener('DOMContentLoaded', (_) => __awaiter(this, void 0, void 0, function* () {
                yield Bug.createModels();
                yield this.createBosses();
                this.canvas.getCanvas();
                this.canvas.setCanvasConfig();
                this.canvas.createGrid({ width: this.blockWidth, height: this.blockHeight, blocks: this.blocks });
                this.createDungeons();
    ...
    getRandomFreeBlock() {
        ...
        if (block.getBug() != null)
            return this.getRandomFreeBlock();
        return block;
    }
    dropBug({ model = null }) {
    ...
    drawCrosshairs() {
        if (this.on || (this.mousePosition.x == null || this.mousePosition.y == null))
            return false;
    ...
    bulletColisionBug(bullet) {
        const coords = bullet.getCoords();
        let colisionLeftBug;
        let colisionRightBug;
        let colisionTopBug;
        let colisionBottomBug;
